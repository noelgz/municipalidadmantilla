	...
	if (is_home() || is_front_page()) {
		...
		if ( is_category() ) {
			...
			while ($parent_id) {
				...
				$breadcrumbs[] = '<li><a href="' . esc_url(get_permalink($page->ID)) . '">' . esc_html(get_the_title($page->ID)) . '</a>';
 ...
 */
function newszine_comment_form_before(){
  add_filter( 'pre_option_comment_registration', 'newszine_theme_add_div_above_form' );
}
add_action( 'comment_form_before', 'newszine_comment_form_before');

function newszine_theme_add_div_above_form( $comment_registration ) {
	// Adjust this to your needs:?>
	<div class="leave-comments">
	<?php remove_filter( current_filter(), __FUNCTION__ );?>
     </div>
	<?php return $comment_registration;
}
